...
Strata writes uploaded files (attachments, exports) to a storage backend. The default `local` backend writes to the container's filesystem via a Docker volume — this works fine for a single container but **files will be inaccessible if you run multiple web replicas**, because each container has its own volume. Switch to a shared backend (cloud object storage or a shared filesystem) whenever you run more than one web container or need uploads to survive container replacement.
