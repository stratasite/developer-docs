...
Set your public URL (e.g. `https://strata.yourcompany.com`) in **Settings → General** —
Strata detects it automatically from the first signed-in visit, and `ASSUME_SSL=true`
ensures the detected protocol is `https` behind your proxy.
...
**Fix:** Set `ASSUME_SSL=true` so Strata trusts the proxy's `X-Forwarded-Proto: https` header and generates `https` URLs. Confirm the public URL in **Settings → General** uses `https`.
