...
This guide is for organizations that manage their own infrastructure and prefer to configure Strata directly rather than using the installer script. It fits container orchestration platforms and multi-node setups where teams want explicit runtime control.

## 0. Get the `.env` Template

Download the official environment template and rename/copy it to `.env`:
...
If preferred, generate the template via installer script:
...
Strata is distributed as a private Docker image on GitLab Container Registry. Use the access token provided with your license purchase:
...
```bash
docker pull registry.gitlab.com/stratado/server:latest
```
