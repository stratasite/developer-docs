...
|------|-----------|-------------|
| **Web** | Thruster → Puma (`WEB_CONCURRENCY` workers × `STRATA_MAX_THREADS` threads) | HTTP RPS and latency |
| **Job** | Solid Queue (`JOB_CONCURRENCY` processes × 3 threads each in `config/queue.yml`) | Job backlog and completion time |
...
```
slots = WEB_CONCURRENCY × STRATA_MAX_THREADS
```

Example: `WEB_CONCURRENCY=2`, `STRATA_MAX_THREADS=5` → **10 slots** per container.
...
Each Puma worker process has its own connection pool per database role. Pool size comes from `config/database.yml`: `STRATA_DB_POOL_SIZE`, or else `STRATA_MAX_THREADS` (default **5**).

```
effective_pool = STRATA_DB_POOL_SIZE ?? STRATA_MAX_THREADS ?? 5
...
```
slots = JOB_CONCURRENCY × threads_per_process
```

Default `threads_per_process` is **3** (`config/queue.yml`).
...
Example: 4 processes × 3 threads = 12 slots; 10s average job duration → ~**1.2 jobs/sec** per container.
...
2. Increase **`JOB_CONCURRENCY`** per task if CPU and DB connections allow.
...
|---------|-----|-----|-------|
| **Single host (Compose)** | 1 container, `WEB_CONCURRENCY=2`, `STRATA_MAX_THREADS=5` | 1 `job` service, `JOB_CONCURRENCY=4` | Installer defaults |
...
5. **Confirm** production web tasks do not process jobs (run a **job** service; no Solid Queue env vars required on web).
...
- [Environment Variables — Performance](/self-hosting/environment-variables#performance)
- [AWS ECS — Job service](/self-hosting/aws-ecs-manual#step-6-add-a-job-ecs-service)
- [Quick Install — Runtime topology](/self-hosting/quick-install#runtime-topology)
