...
:::note
This is a **Sheets export integration, not a sign-in method**. It authorizes Strata to write to a user's Google Sheets — it does not let users log in with Google. To configure login, see [Authentication & SSO](/self-hosting/authentication) (Google works as an [OpenID Connect](/self-hosting/authentication/oidc) provider).
:::
...
- The public URL in **Settings → General** matches the URL users use to access Strata.
...
Also confirm the public URL in **Settings → General**, your reverse proxy, and the browser URL all agree on the same public host and protocol.
