...
If you want to pre-populate configuration before running the installer, download the template file:

```bash
curl -fsSL https://strata.do/server/.env.example -o .env.example
```

You can also generate it using the installer script itself:
...
```bash
docker compose logs --tail 30 web jobs
```
