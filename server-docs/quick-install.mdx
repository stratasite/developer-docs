...
Open `http://your-server:3000` (or your configured port) and complete the Setup Wizard to create the first admin account.
...
The installer creates two services from the same image:

- `web` (application traffic)
- `worker` (background processor)

For orchestrated deployments that require a single task/container, you can run the same image with `STRATA_PROCESS_TYPE=all` (or unset) to run both roles in one runtime.
...
```bash
docker compose logs --tail 30 web worker
```
