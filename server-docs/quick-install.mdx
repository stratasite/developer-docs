...
Jobs run in the separate `job` service (production layout). Tune `WEB_CONCURRENCY`, `JOB_CONCURRENCY`, and related vars in `.env` — see [Environment Variables](/self-hosting/environment-variables#performance).
