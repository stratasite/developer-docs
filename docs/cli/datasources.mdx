...
- A Strata project initialized (see [Projects](/developer-guide/cli/projects))
...
If you omit the datasource key, the CLI prompts you to choose one. You can search and filter tables, then select one (e.g. to copy the name for use with [Creating Tables](/developer-guide/cli/tables)).
...
When adding or editing a datasource, you can set a **tier** (`hot`, `warm`, or `cold`). Tiers influence query routing and performance. For a full explanation, see [Datasources](/developer-guide/semantic-model/datasources#tier-configuration).
...
3. **Set appropriate tiers** - Match performance characteristics (see [Datasources](/developer-guide/semantic-model/datasources#tier-configuration))
...
- [Building Your Model](/developer-guide/cli/building-your-model) - Turn these tables into a semantic model, yourself or with a coding agent
- [Creating Tables](/developer-guide/cli/tables) - Create semantic table models from datasource tables
- [Datasources (tiers, full config)](/developer-guide/semantic-model/datasources)
- [Adapters](/developer-guide/semantic-model/adapters) - Adapter-specific configuration
