...
**Rule you must respect:** All measures in a single query must come from the same datasource. Cross-datasource measure queries need [extended blending](/advanced/extended-blending-groups).
...
When two tables expose the same measure (e.g. "Store Sales Price") via the [unique naming principle](/getting-started/concepts#core-design-principles), the planner prefers the lower-cost table when both can satisfy the query.

### Partition Constraints

**Where:** In the table's YAML under `partition`.  
**What it means:** This table holds data for a specific range (e.g. "Sale Date ≥ 2024-01-01"). The planner prefers tables whose partition matches the query filters so it can prune data. See [Partitions](/advanced/partitions) for full syntax.
