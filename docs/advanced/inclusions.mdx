...
Inclusions solve a critical problem: **computing correct averages and medians when counts per group are uneven**. They force calculations at a finer grain (by including extra dimensions) before rolling up to the final result. This is the opposite direction to [exclusions](/advanced/exclusions), which remove dimensions from calculations.
