...
Steps 4 and 5 below build tables one at a time. You can also open the project in a coding agent and have it author the whole model from the `AGENTS.md` that `strata init` created. See [Building Your Model](/developer-guide/cli/building-your-model).
:::
...
- [Tutorial](/developer-guide/getting-started/tutorial) — Hands-on walkthrough with TPC-DS
- [Building Your Model](/developer-guide/cli/building-your-model) — Build the model yourself or with a coding agent
- [CLI Reference](/developer-guide/cli) — Detailed command documentation
- [Concepts](/developer-guide/getting-started/concepts) — Understand the semantic layer
