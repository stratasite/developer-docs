...
If you have a Strata server running locally (see [Quick Install](/self-hosting/quick-install)), the fastest way to get started is:
...
The TPC-DS dataset is bundled in the Strata Docker image — no separate database setup needed. Find your API key on your project page in the Strata web app.
...
- **Strata CLI** installed ([Installation](/developer-guide/getting-started/installation))
- Familiarity with [Core concepts](/developer-guide/getting-started/concepts) (tables, relationships, datasources)
...
Replace `/path/to/your/tpcds.duckdb` with the actual path to the `tpcds.duckdb` file you created in Step 1. For more options, see the [DuckDB adapter](/developer-guide/semantic-model/adapters/duckdb) reference.
...
Get the API key from your Strata web app: open your project, go to **Settings → API Keys**, and create or copy a key. See [Deployment](/developer-guide/cli/deployment) for more.
...
See [Deployment](/developer-guide/cli/deployment) for more details on server URL configuration, API keys, and Git requirements.
...
- [TPC-DS Tutorial (Examples)](/developer-guide/examples/tpcds-tutorial) — Deeper dive into the TPC-DS model and patterns
- [Star schema pattern](/developer-guide/examples/patterns/star-schema) — How fact and dimension tables relate
- [CLI Reference](/developer-guide/cli) — All Strata CLI commands
- [Semantic Model](/developer-guide/semantic-model/tables) — Tables, fields, and relationships in detail
