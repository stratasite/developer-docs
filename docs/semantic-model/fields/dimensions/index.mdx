...
The data type of the dimension. See [data types](/semantic-model/fields/data-types).

```yaml
data_type: string
```

For **date and date_time** dimensions, including grains and filtering patterns, see [Date and DateTime dimensions](/semantic-model/fields/dimensions/date-time).
...
[Learn about formatters →](/semantic-model/fields/formatters)
...
For dimensions that should blend across tables in queries, set the same `extended_blend_group` on each. See [Extended blending](/semantic-model/expressions/extended-blending) and [Extended blending groups](/advanced/extended-blending-groups).
...
- Evaluation happens at query time, similar to [compound measures](/semantic-model/fields/measures/compound).
...
4. **Set grains for date/datetime** - See [Date and DateTime dimensions](/semantic-model/fields/dimensions/date-time)
5. **Mark primary keys** - Improve query optimization
6. **Use appropriate data types** - Match database column types

## Next Steps

- [Date and DateTime dimensions](/semantic-model/fields/dimensions/date-time) - Grains, formatting, and date-specific patterns
