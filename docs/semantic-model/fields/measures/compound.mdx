...
When a compound measure is used inside another expression (e.g. in a [temporal decorator](/developer-guide/advanced/decorators/temporal) or another compound measure), it is also resolved at query time. The dependency chain is evaluated when the query is built, so the order of definition in YAML does not need to follow the dependency order.
...
- [Temporal decorators](/developer-guide/advanced/decorators/temporal) — Compound measures in time-based calculations
