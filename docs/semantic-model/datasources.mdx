...
For CLI commands to add, test, and manage datasources, see [Managing Datasources](/cli/datasources).
...
1. **PostgreSQL** — [postgres](/semantic-model/adapters/postgres)
2. **Snowflake** — [snowflake](/semantic-model/adapters/snowflake)
3. **MySQL** — [mysql](/semantic-model/adapters/mysql)
4. **SQL Server** — [sqlserver](/semantic-model/adapters/sqlserver)
5. **Athena** — [athena](/semantic-model/adapters/athena)
6. **Trino** — [trino](/semantic-model/adapters/trino)
7. **DuckDB** — [duckdb](/semantic-model/adapters/duckdb)
8. **Druid** — [druid](/semantic-model/adapters/druid)
...
- Universes and query plans are built per datasource, then optionally merged at the result level (see [Multi-Datasource](/advanced/multi-datasource)).
...
- [Managing Datasources](/cli/datasources) — CLI commands
- [Adapters](/semantic-model/adapters) — Adapter-specific configuration
